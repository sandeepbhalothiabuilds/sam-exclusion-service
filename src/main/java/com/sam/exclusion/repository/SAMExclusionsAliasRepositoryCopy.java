...
import com.sam.exclusion.entity.SAMExclusionsAliasCopy;
...
@Repository
public interface SAMExclusionsAliasRepositoryCopy extends JpaRepository<SAMExclusionsAliasCopy, Long> {

    @Modifying
    @Transactional
    @Query(value = "delete FROM sam.sam_exclusions_alias_copy WHERE sam_exclusion_data_id not in (select sam_exclusion_data_id from sam.sam_exclusions_data_copy)", nativeQuery = true)
