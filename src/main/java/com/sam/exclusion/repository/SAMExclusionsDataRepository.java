...
public interface SAMExclusionsDataRepository extends JpaRepository<SAMExclusionsData, Long> {
    ...
    @Query(value = "select * from sam.sam_exclusions_data sed where (sed.sam_number in (:samNumber) and sed.sam_number <> '') or (sed.unique_entity_id in (:uniqueEntityID) and sed.unique_entity_id <> '') limit 2000;", nativeQuery = true)
    ...
    @Query(value = "select * from sam.sam_exclusions_data sed where upper(sed.full_address) like upper( CONCAT('%', :address, '%')) limit 2000", nativeQuery = true)
    List<SAMExclusionsData> findRecordsByAddress(@Param("address") String address);
}
